...
// 회귀: 가동 = 실가동(574 A-ON) "그리고" 릴레이(35=1) 둘 다. 각각 가끔 값이 굳어 오탐하므로
// 둘 다 요구해 서로를 걸러낸다. controlunit 1 = 3동.
{
  const rawX = [
    [1, 17, 25, 2800], [1, 18, 25, 2750], [1, 28, 25, 0], [1, 35, 25, 1], [1, 574, 25, 4], // 팬8: relay1+574=4 → 가동
    [1, 17, 24, 3000], [1, 18, 24, 2950], [1, 28, 24, 0], [1, 35, 24, 1], [1, 574, 24, 2], // 팬7: relay1인데 574=2 → 정지(574가 걸러냄)
    [1, 17, 21, 2900], [1, 18, 21, 2850], [1, 28, 21, 0], [1, 35, 21, 0], [1, 574, 21, 4], // 팬4: 574=4인데 relay0 → 정지(relay가 걸러냄)
    [1, 17, 27, 3250], [1, 18, 27, 3200], [1, 28, 27, 0], [1, 35, 27, 0], [1, 574, 27, 4], // 팬10: 574=4·relay0 → 정지
    [1, 17, 128, 3350], [1, 18, 128, 3300], [1, 28, 128, 0], [1, 35, 128, 1], [1, 574, 128, 4], // 열풍기1: 둘 다 → 가동
    [1, 17, 129, 3350], [1, 18, 129, 3300], [1, 28, 129, 0], [1, 35, 129, 1], [1, 574, 129, 2], // 열풍기2: 574=2 → 정지
  ]
  ...
  check('회귀: 574=4 + relay=1 둘 다면 가동 (팬8)', run('터널팬 8') === true)
  check('회귀: relay=1이어도 574=2면 정지 (팬7)', run('터널팬 7') === false)
  check('회귀: 574=4여도 relay=0이면 정지 (팬4)', run('터널팬 4') === false)
  check('회귀: 574=4·relay0 정지 (팬10·핵심)', run('터널팬 10') === false)
  check('회귀: 열풍기 둘 다 가동 / 574=2 정지', run('열풍기 1') === true && run('열풍기 2') === false)
  check('회귀: 3동 가동 터널팬 1대(팬8만)', hX.fansRunning === 1)
}
