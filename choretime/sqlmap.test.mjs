...
// 회귀: 가동 판정은 실제 릴레이 상태(descfk35=1). descfk574(A-ON)는 정지 후에도 남을 수 있어
// 신뢰 불가(3동 10번: 574=4인데 relay35=0 → 정지가 맞음). controlunit 1 = 3동.
{
  const rawX = [
    [1, 17, 25, 2800], [1, 18, 25, 2750], [1, 28, 25, 0], [1, 35, 25, 1], [1, 574, 25, 4], // 터널팬8: relay=1 → 가동
    [1, 17, 24, 3000], [1, 18, 24, 2950], [1, 28, 24, 0], [1, 35, 24, 1], [1, 574, 24, 4], // 터널팬7: relay=1 → 가동
    [1, 17, 27, 3250], [1, 18, 27, 3200], [1, 28, 27, 0], [1, 35, 27, 0], [1, 574, 27, 4], // 터널팬10: 574=4(낡음)이지만 relay=0 → 정지(핵심)
    [1, 17, 20, 3400], [1, 18, 20, 3350], [1, 28, 20, 0], [1, 35, 20, 0], [1, 574, 20, 2], // 터널팬3: relay=0 → 정지
    [1, 17, 128, 3350], [1, 18, 128, 3300], [1, 28, 128, 0], [1, 35, 128, 1], [1, 574, 128, 4], // 열풍기1: relay=1 → 가동
    [1, 17, 129, 3350], [1, 18, 129, 3300], [1, 28, 129, 0], [1, 35, 129, 0], [1, 574, 129, 2], // 열풍기2: relay=0 → 정지
  ]
  ...
  check('회귀: relay=1 이면 가동 (터널팬8)', run('터널팬 8') === true)
  check('회귀: relay=1 이면 가동 (터널팬7)', run('터널팬 7') === true)
  check('회귀: 574=A-ON(낡음)이라도 relay=0이면 정지 (터널팬10·핵심)', run('터널팬 10') === false)
  check('회귀: relay=0이면 정지 (터널팬3)', run('터널팬 3') === false)
  check('회귀: 열풍기 relay=1 가동 / relay=0 정지', run('열풍기 1') === true && run('열풍기 2') === false)
  check('회귀: 3동 가동 터널팬 2대(7·8번, 10번 제외)', hX.fansRunning === 2)
}
