...
const raw = [
  ...
  // 실가동: 터널팬4(idx21)=574값4 켜짐, 터널팬1(idx18)=574값2 정지.
  // 순환팬(idx68)은 574값 없이(0) descfk28==5만으로 가동 판정돼야 함 (3동 사례 재현)
  [3, 574, 21, 4], [3, 574, 18, 2],
...
// 회귀: 가동 판정은 오직 실가동(descfk574)=A-ON 으로. 정상적으로 도는 터널팬도 code28=0 이므로
// 모드로 거르면 안 된다(그렇게 하면 도는 5대를 놓쳤던 3동 사례). controlunit 1 = 3동.
{
  const rawX = [
    [1, 17, 21, 2900], [1, 18, 21, 2850], [1, 28, 21, 0], [1, 574, 21, 4], // 터널팬4: 모드0인데 574=A-ON → 가동(핵심)
    [1, 17, 25, 2800], [1, 18, 25, 2750], [1, 28, 25, 0], [1, 574, 25, 4], // 터널팬8: 모드0·574=A-ON → 가동
    [1, 17, 20, 3200], [1, 18, 20, 3150], [1, 28, 20, 0], [1, 574, 20, 2], // 터널팬3: 574=A-OFF → 정지
    [1, 17, 23, 3500], [1, 18, 23, 3450], [1, 28, 23, 3], [1, 574, 23, 2], // 터널팬6: 최소환기 모드지만 574=A-OFF(꺼진 구간) → 정지
    [1, 17, 128, 3350], [1, 18, 128, 3300], [1, 28, 128, 0], [1, 574, 128, 4], // 열풍기1: 574=A-ON → 가동
    [1, 17, 129, 3350], [1, 18, 129, 3300], [1, 28, 129, 0], [1, 574, 129, 2], // 열풍기2: 574=A-OFF → 정지
  ]
  ...
  const run = (n) => hX.stages.find((s) => s.name === n)?.running
  check('회귀: 모드0이어도 574=A-ON이면 가동 (터널팬4·정상 가동 팬)', run('터널팬 4') === true)
  check('회귀: 574=A-ON 가동 (터널팬8)', run('터널팬 8') === true)
  check('회귀: 574=A-OFF면 정지 (터널팬3)', run('터널팬 3') === false)
  check('회귀: 최소환기 모드라도 574 꺼짐구간이면 정지 (터널팬6)', run('터널팬 6') === false)
  check('회귀: 열풍기 574=A-ON 가동 / A-OFF 정지', run('열풍기 1') === true && run('열풍기 2') === false)
  check('회귀: 3동 가동 터널팬 2대(4·8번)', hX.fansRunning === 2)
}
